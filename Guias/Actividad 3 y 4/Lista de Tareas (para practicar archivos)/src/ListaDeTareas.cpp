...
void ListaDeTareas::mostrarTareaConEti(int id){
    for (int i=0 ; i<_cantTareas ; i++ ){
        if(_listaTareas[i].getId() == id ){
            ...
            cout << setw(15) << setiosflags(ios::left) << _listaTareas[i].getEstado() << endl;
            cout << "ETIQUETAS: " << endl;
            string *p = _listaTareas[i].getVecEtiquetas();
            for (int j=0 ; j<_listaTareas[i].getCantEti() ; j++ ){
                cout << j+1 << " : " << p[j]<< endl;
            }
...
void ListaDeTareas::mostrarTareaPorEtiqueta(string etiqueta){
    string *p;
    bool esta = false;
    for (int i=0 ; i<_cantTareas ; i++ ){
        esta = false;
        for (int j=0 ; j<_listaTareas[i].getCantEti() ; j++ ){
            p = _listaTareas[i].getVecEtiquetas();
            if(p[j] == etiqueta){
                esta = true;
            }
        }
        if(esta){
            cout << setw(3) << setiosflags(ios::left) << _listaTareas[i].getId();
            cout << setw(30) << setiosflags(ios::left) << _listaTareas[i].getDescripcion() << endl;
